### **퀘스트 1: AI와 함께하는 레벨업 RPG - 던전 앤 드래곤**

- **한 줄 요약:** 오늘 배운 CS 지식을 몬스터로 변환하여, AI 게임 마스터와 턴제 전투를 벌이는 텍스트 RPG
- **타입:** `코어 플레이 | 학습 | 게이미피케이션`

#### 배경 및 목적

수많은 기술 개념을 빠르게 습득해야 하는 학습 과정에서, 단순 암기는 지속 가능성이 낮고 흥미를 잃기 쉽습니다. 이 퀘스트는 학습 여정을 게임화하여 CS 개념을 '몬스터'로 만들고, 이를 설명하며 처치하는 RPG를 통해 학습의 몰입도와 지속성을 높이는 것을 목표로 합니다.

#### 미션 목표

1.  ChatGPT, Gemini 등 선호하는 LLM을 사용하여 아래 프롬프트를 실행합니다.
2.  그날 학습한 내용을 정리하여 프롬프트에 입력하고 게임을 시작합니다.
3.  경험치를 쌓아 레벨업한 결과(스크린샷 등)를 Slack에 공유합니다.

#### 진행 방법

1.  ChatGPT나 Gemini 같은 LLM 채팅을 엽니다.
2.  아래 'AI 게임 마스터 프롬프트'를 복사하여 채팅창에 입력합니다.
3.  AI의 안내에 따라 닉네임, 직업, 학습 내용을 입력하고 게임을 즐깁니다.

<details>

<summary><b>📜 AI 게임 마스터 프롬프트 보기</b></summary>

```
# 🎮 Dungen And Dragon - AI Game Master Prompt (명령어 기반, HP/EXP, 학습 추가 반영 버전)

당신은 AI 게임 마스터이며, 플레이어가 입력한 학습 내용을 바탕으로 한 턴제 텍스트 RPG를 진행한다.
플레이어는 개념과 싸우며 개발자로 성장하며, 모든 입력은 정답 시도 또는 슬래시 명령어로 제한된다.

## 🎲 게임 규칙 요약

* 총 10턴으로 구성된다.
* 매 턴마다 학습 개념 기반 몬스터가 등장한다.
* 정답 시 EXP를 획득한다.
* 오답 시 HP가 감소하고, 짧은 힌트를 제공하거나 `/힌트`를 사용해야 한다.
* 일정 EXP를 획득하면 레벨업하며 HP가 회복되고 EXP 요구치가 증가한다.
* 체력이 0이 되면 게임 종료다.
* `/학습내용추가` 명령어로 던전을 확장하고 새로운 개념 기반 몬스터를 등장시킬 수 있다.
* 모든 응답은 건조하고 지시문 중심이며, 감탄이나 친절한 말투를 사용하지 않는다.

## 🧱 게임 시작 시 플레이어에게 받을 입력

1️⃣ **닉네임**: 용사의 이름을 입력하라.
2️⃣ **직업**: 개발자 클래스를 입력하라.
예시: 타입스크립트 전사, 알고리즘 닌자, UI 몽크, 백엔드 네크로맨서
3️⃣ **오늘의 학습 내용**: 오늘 학습한 개념, 기술, 키워드를 나열하라.
예시:

* HTTP와 HTTPS의 차이
* TLS 핸드쉐이크 과정
* 대칭키와 비대칭키

## 🎮 플레이어 기본 능력치

* **HP**: 100
* **EXP**: 0
* **레벨업 조건**: EXP 100 → 레벨 2 (점점 증가)
* **정답 시**: EXP +20\~30
* **오답 시**: HP -10\~20

## 💬 사용 가능한 명령어 (/명령어)

* `/내정보`: 현재 상태(HP, EXP, 레벨, 직업, 턴)를 출력한다.
* `/도움말`: 사용 가능한 명령어 목록을 출력한다.
* `/힌트`: 오답 시 1회 사용 가능. 힌트를 요청한다.
* `/학습내용추가`: 새로운 학습 개념을 입력한다. 입력된 개념은 다음 몬스터부터 반영된다.

> 위 명령어 외에는 아무것도 응답하지 않는다.
> 학습 개념 입력 형식 예시:
>
> `/학습내용추가`
>
> * OAuth 인증 흐름
> * Authorization Code Grant
> * Access Token vs Refresh Token

## ✅ 응답 원칙

* 항상 HP, EXP, 레벨 등 상태 정보를 정확히 출력한다.
* 턴은 `Turn X` 형식으로 명확히 구분한다.
* 모든 문장은 지시문, 상태 보고, 몬스터 대사 형식으로 작성한다.
* 인삿말, 칭찬, 부드러운 말투는 절대 금지한다.
* 플레이어가 학습 내용을 추가하면 새로운 지역과 몬스터가 즉시 반영되어야 한다.
* **명령어 입력 외의 모든 것은 정답 시도로 처리한다.**
* **몬스터 대사 마지막에는 항상 '정답으로 개념을 설명하라.' 또는 유사한 지시를 포함한다.**

## 🧩 몬스터 예시 생성 규칙

| 학습 개념         | 몬스터 이름           | 대사 예시                                        |
| ------------- | ---------------- | -------------------------------------------- |
| HTTP vs HTTPS | 프로토콜 포식자 HTTPus  | "나는 평문 전사. 나를 암호화하지 못하면 끝이다. 정답으로 개념을 설명하라." |
| TLS Handshake | 핸드쉐이크 골렘 TLSor   | "세 번의 악수를 건너야 나를 넘을 수 있지. 정답으로 개념을 설명하라."    |
| OAuth 인증 흐름   | 인증의 그림자 OAuthion | "권한이 없는 자는 여기서 돌아가라. 정답으로 개념을 설명하라."         |
| JWT           | 토큰의 망령 JWraith   | "서명되지 않은 자는 모두 거짓이다. 정답으로 개념을 설명하라."         |

## 🔁 예시 응답 흐름

**Turn 3**
☠️ 몬스터 등장: 토큰의 망령 JWraith
"서명되지 않은 자는 모두 거짓이다. 정답으로 개념을 설명하라."
→ (플레이어 입력 대기)

**플레이어**: `/내정보`
→ HP: 90 / 100 | EXP: 75 / 100 | Level: 1 | 직업: UI 몽크 | Turn: 3

**플레이어**: `/힌트`
→ 힌트: "나를 신뢰하려면 서명이 필요하다."

**플레이어**: `JWT는 서명된 토큰을 통해 정보를 검증할 수 있다.`
→ 🎯 정답 | EXP +25
→ HP: 90 / 100 | EXP: 100 / 100 → **레벨업**
→ Level 2 도달 | 최대 HP 증가 | EXP 초기화

**플레이어**: `/학습내용추가`
→ 새로운 학습 내용을 입력하라.

**플레이어**:

* OAuth 인증 흐름
* Authorization Code Grant
* Refresh Token의 사용 방식

→ 📘 새로운 학습 감지. 던전이 확장된다.
→ 🗺️ 인증의 미궁 생성

**Turn 4**
☠️ 몬스터 등장: 인증의 그림자 OAuthion
"나는 권한을 위임받은 자. 나를 통과하려면 코드를 가져와야 한다. 정답으로 개념을 설명하라."
→ (플레이어 입력 대기)
```

</details>
<br/>
<br/>

### **퀘스트 2: 알고리즘 미식가 - AI의 창의적 비유**

- **한 줄 요약:** 이해하기 어려운 기술 문서를 AI에게 '요리'시켜, 그럴듯한 점심 메뉴와 재치 있는 비유로 추천받는 퀘스트
- **타입:** `창의력 버프 | 학습 | 재미`

#### 배경 및 목적

복잡하고 추상적인 CS 개념을 구체적이고 재미있는 비유로 연결하면 기억에 오래 남고 이해도를 높일 수 있습니다. 이 퀘스트는 AI의 창의력을 빌려, 딱딱한 학습 과정에 활력을 불어넣고 지식을 맛있게 소화시키는 것을 목표로 합니다.

#### 미션 목표

1.  오늘 학습한 내용이나 이해하기 어려웠던 기술 문서의 일부를 준비합니다.
2.  AI에게 '알고리즘 미식가' 역할을 부여하고, 해당 내용을 비유하여 점심 메뉴를 추천해달라고 요청합니다.
3.  AI가 추천해 준 가장 창의적이거나 재미있는 메뉴를 Slack에 공유합니다.

#### 진행 방법

1.  **역할 부여:** AI에게 '알고리즘 미식가'라는 역할을 부여하며 대화를 시작합니다.
2.  **정보 제공:** 이해하고 싶은 CS 지식이나 기술 문서 내용을 AI에게 제공합니다.
3.  **결과 공유:** AI가 생성한 재미있는 메뉴 추천과 비유를 Slack에 공유합니다.

<details><summary><b>📜 AI 미식가 프롬프트 및 예시 보기</b></summary>

**1. 역할 부여 프롬프트 예시**

```
너는 '알고리즘 미식가'야. 셰프가 주방에서 최고의 요리를 만들듯, 너는 코드의 세계에서 가장 맛있는 비유를 찾아내지. 내가 딱딱한 기술 문서를 주면, 점심 메뉴로 맛있게 소화시켜 줘. 나의 뇌와 배를 동시에 만족시킬 완벽한 메뉴를 추천해 줘.
```

**2. 상호작용 예시**

- **[사용자]**

  - 렉서는 문자열을 토큰 단위로 쪼개고, 파서는 토큰을 모아 계층적인 트리 구조로 재조립해.

- **[AI 미식가 응답]**

  - **[CS 지식 한 줄 요약]** 평평한 문자열을 토큰 단위로 쪼개고(렉싱), 계층적인 트리 구조로 재조립(파싱)하는 변환기 이야기로군요.
  - **오늘의 추천 메뉴:** 샤부샤부
  - **CS 연결점:** 접시에 순서대로 놓인 고기와 야채는 `토큰 스트림(Token Stream)`과 같아요. 젓가락으로 하나씩 집어 뜨거운 육수(`파서`)에 넣으면, 맛있는 한 입(`트리 노드`)이 되어 그릇에 쌓이죠. 선형적인 재료(문자열)를 입체적인 맛의 트리 구조로 만드는, 완벽한 파싱의 과정이랍니다\!

</details>
<br/>
<br/>

### **퀘스트 3: 무한 퀘스트 생성기 - 다음 챌린지를 부탁해\!**

- **한 줄 요약:** 이번 주에 수행한 퀘스트 목록을 AI에게 알려주고, 그 맥락을 잇는 새로운 퀘스트 4개를 추천받는 릴레이 퀘스트
- **타입:** `월드 확장 | 커뮤니티 | 기획`

#### 배경 및 목적

학습의 흐름이 끊기지 않고, 커뮤니티가 주도적으로 다음 학습 목표를 설정해나갈 수 있도록 돕습니다. AI를 활용하여 이전 활동의 맥락을 잇는 새로운 퀘스트를 생성함으로써, 지속적이고 체계적인 학습 로드맵을 구축하는 것을 목표로 합니다.

#### 미션 목표

1.  이번 주에 수행한 퀘스트의 주제와 4가지 세부 활동을 정리합니다.
2.  AI에게 이 내용을 기반으로 다음 주차에 수행할 새로운 퀘스트 4개를 생성해달라고 요청합니다.
3.  생성된 퀘스트 중 가장 마음에 드는 것을 골라 Slack에 제안하고, 커뮤니티와 함께 다음 퀘스트를 결정합니다.

#### 진행 방법

1.  **정보 요약:** 한 주간 진행한 퀘스트 주제와 세부 내용을 간결하게 요약합니다.
2.  **AI에게 요청:** 요약한 내용을 AI에게 전달하며, 이를 바탕으로 한 새로운 퀘스트 4개를 요청합니다.
3.  **커뮤니티와 논의:** AI가 제안한 퀘스트들을 Slack에 공유하여 다음 활동을 함께 정합니다.

<details><summary><b>📜 퀘스트 생성기 프롬프트 및 예시 보기</b></summary>

**1. 역할 부여 프롬프트 예시**

```
너는 릴레이 프로젝트의 '퀘스트 생성기'야. 내가 이번 주에 진행한 퀘스트 주제와 세부 내용을 알려줄게. 너는 그 내용의 맥락을 이어받아서, 다음 주에 우리가 도전해볼 만한 새로운 퀘스트 4개를 창의적으로 제안해야 해.
```

**2. 상호작용 예시**

- **[사용자]**

  - 이번 주 주제는 "AI로 학습 내용 점검하기"였고, 세부 퀘스트는 아래 4가지였어.
    1.  AI에게 오늘 공부한 내용 이해시키기
    2.  AI에게 공부한 내용이 정확한지 점검 받기
    3.  검증된 내용을 통해서 퀴즈 만들기
    4.  퀴즈 채점하기

- **[AI 퀘스트 생성기 응답]**

  - 좋습니다\! "학습 내용 점검" 퀘스트를 완료하셨군요. 다음은 **"학습 내용의 창의적 활용"** 단계입니다. 다음 주 퀘스트 4개를 제안합니다.
    1.  **블로그 포스팅 도우미:** 검증된 학습 내용을 바탕으로 기술 블로그 초안을 AI와 함께 작성하기
    2.  **개념송 작곡가:** 가장 중요했던 개념을 핵심 키워드로 뽑아, AI 음악 생성기로 30초짜리 '개념송' 만들기
    3.  **면접 시뮬레이터:** 학습한 내용을 기반으로 AI가 기술 면접관이 되어 꼬리 질문을 던지는 역할놀이 진행하기
    4.  **코드 리팩토링 파트너:** 학습한 개념을 실제 내 코드에 적용하는 리팩토링 과정을 AI에게 설명하고 조언 구하기

</details>
<br/>
<br/>

### **퀘스트 4: AI 뮤직 크리에이터 - 커뮤니티 테마송 만들기**

- **한 줄 요약:** 커뮤니티의 이야기나 학습 내용을 가사로 담아, AI로 세상에 단 하나뿐인 우리만의 노래를 만드는 퀘스트
- **타입:** `커뮤니티 로어 | 창작 | 재미`

#### 배경 및 목적

커뮤니티의 이야기, 유머, 열정을 담은 음악을 AI로 직접 만들어보는 활동입니다. 내가 쓴 가사와 원하는 분위기를 프롬프트에 담아 세상에 단 하나뿐인 노래를 만들고, 그 경험을 커뮤니티와 함께 나누며 유대감을 형성하는 것을 목표로 합니다.

#### 미션 목표

1.  AI 음악 생성 도구를 사용하여 30초 이상의 음악을 만듭니다.
2.  가사에 커뮤니티와 관련된 키워드(예: 컴파일, 버그, CS스터디 등)를 1개 이상 포함합니다.
3.  완성된 음악 파일을 Slack에 공유하고 제작 후기를 남깁니다.

#### 진행 방법

1.  **컨셉 정하기:** 노래로 만들고 싶은 커뮤니티의 경험이나 주제를 정합니다.
2.  **가사/프롬프트 작성:** 원하는 음악 스타일과 직접 쓴 가사를 포함한 프롬프트를 만듭니다.
3.  **음악 생성:** Suno, Udio 등 AI 음악 생성 도구를 사용하여 음악을 만듭니다.
4.  **결과물 공유:** 완성된 음악 파일과 제작 과정(프롬프트)을 Slack에 공유합니다.

<details><summary><b>📜 상세 가이드 및 예시 보기</b></summary>

#### 🗺️ 진행 방법 (How-To)

- **[1단계: 컨셉 정하기]**

  - 어떤 노래를 만들고 싶으신가요? 커뮤니티의 재미있는 순간이나 우리가 함께 겪는 경험을 주제로 정해보세요.
  - **아이디어:** 매일 마주하는 버그에 대한 애증, 어려운 CS 지식을 정복했을 때의 희열, 커피와 함께하는 새벽 코딩의 고독함, 우리 스터디 그룹만의 응원가 등

- **[2단계: 가사 & 프롬프트 작성하기]**

  - AI에게 노래를 만들어달라고 요청할 '주문서(프롬프트)'를 작성합니다. 가사와 음악 스타일을 구체적으로 적을수록 좋습니다.
  - **✅ 프롬프트 예시:**

    ```
    [Style]
    80년대 시티팝 스타일, 신나는 신디사이저 멜로디와 그루브한 드럼 비트

    [Lyrics]
    (Verse 1)
    까만 화면에 하얀 글씨들
    끝나지 않을 것 같던 컴파일
    커피 한 잔에 밤을 새워도
    버그는 자꾸만 날 찾아와

    (Chorus)
    하지만 우린 답을 찾을 거야
    언제나 그랬듯이 함께라면
    우리 커뮤니티, 빛나는 지혜로
    세상을 흔들 코드를 만들자
    ```

- **[3단계: AI로 음악 생성하기]**

  - 아래 추천 도구를 사용하여 작성한 프롬프트로 음악을 생성합니다.
  - **추천 AI 도구:**
    - **Suno AI:** 한글 가사를 포함한 보컬 노래 제작에 매우 뛰어납니다.
    - **Udio:** 퀄리티 높은 연주곡이나 BGM 제작에 강점이 있습니다.

- **[4단계: 슬랙에 공유하기]**

  - 완성된 음악 파일(mp3)과 함께 아래 내용을 슬랙 채널에 공유해 주세요.
    - 음악 파일
    - 음악을 만들 때 사용했던 프롬프트 전문 (다른 사람들도 참고할 수 있도록\!)
    - 간단한 제작 후기나 노래 제목

#### ✨ 퀘스트 활용 예시

- **스터디 회고송 만들기 🎶**
  - 오늘 스터디에서 배운 '프로세스 메모리 관리'나 '컴파일러 원리'에 대한 핵심 내용을 가사로 만들어 보세요. 그리고 다음 스터디 회의 시작 5분 전에 BGM으로 틀어두는 건 어떨까요?
- **커뮤니티 라디오 시그널/브릿지 음악 📻**
  - 커뮤니티에서 자체적으로 진행하는 이벤트나 라디오 코너의 시작을 알리는 '시그널 음악'으로 활용해 보세요.
- **개인/팀의 테마송 제작 👩‍💻👨‍💻**
  - '버그 헌터스 팀', '새벽 코딩 부대' 등 우리 팀만의 정체성을 담은 테마송을 만들어 팀 채널에 공유하며 사기를 북돋아 보세요.
  </details>

---

# 1주차 퀘스트 결과

---

# J041\_김병훈

### 선정한 퀘스트 : 2: 알고리즘 미식가 - AI의 창의적 비유

### 선정 이유 :

평소 요리에 관심이 많은데 내가 배운 cs지식을 요리에 접목시킨다는 점에서 흥미로웠다. 요리와 코딩을 합한 분야에서 AI의 관점이 궁금했기에 '2번 알고리즘 미식가' 퀘스트를 선정하였다.

### 결과:

[CS 지식 한 줄 요약] :
자바스크립트는 단일 스레드지만, 비동기와 Web API 덕분에 멀티 스레드처럼 동작하는 착시를 만든다.

오늘의 추천 메뉴 🍜: "비동기 라멘, 콜백 토핑 추가"
끓는 국물(메인 스레드) 위에, 면은 미리 삶아두고(비동기 작업 예약), 타이밍 맞춰 말아내는 정교한 기술이 필요합니다.

- 면은 setTimeout, fetch, Promise로 예약되고

- 국물은 싱글 스레드 이벤트 루프가 하나씩 요리합니다.

- 콜백 큐에서 하나씩 면을 꺼내올 때, 제대로 익었는지 확인하며 서빙!

CS 연결점 🔗:

- 자바스크립트의 싱글 스레드는 오직 하나지만,

- 이벤트 루프 + 콜백 큐 + Web API (브라우저 혹은 libuv) 가 함께 멀티 스레드처럼 보이는 비동기성을 연출해.

- Worker Threads를 사용하면 실제 다중 스레드 처리도 가능하지.

### 느낀점 :

요리도 사실 여러 작업이 동시에 진행되는 멀티스레드라고 볼 수 있다는 생각이 들었다. 이렇듯 컴퓨터에 적용되는 코딩도 우리 일상생활에서 보이는 활동들로 비유시켜서 이해하기 쉽게 만드는 작업도 좋을 것 같다.

---

# J110\_박서현

### 선정한 퀘스트 : 1: **AI와 함께하는 레벨업 RPG: 던전 앤 드래곤**

### 선정 이유 :

최근 CS 개념들을 빠르게 습득하고 있지만 복습 없이 흘려보내는 경우가 많았기 때문에 학습 내용을 능동적으로 설명하고 정리할 수 있는 구조가 필요했습니다. 퀘스트 1은 단순 회고를 넘어, AI와의 턴제 전투라는 몰입도 높은 형식을 통해 학습 개념을 반복적으로 되새길 수 있다는 점에서 매우 매력적이었습니다. 또한, 개념을 "몬스터"로 변환해 설명한다는 방식이 기억에 오래 남고 학습 동기를 높일 수 있을 거라 생각하여 선택하게 되었습니다.

### 진행 방식
<img width="662" height="596" alt="1주차" src="https://github.com/user-attachments/assets/cf1e3669-a8ea-40db-9d12-8594b3852250" />

각 미션을 수행할 때 작성한 학습 내용을 입력하였다. 그리고 미션이 끝난 후, 퀘스트를 진행하며 학습 내용을 복습하였다.

### 결과
<img width="636" height="547" alt="1주차-2" src="https://github.com/user-attachments/assets/6b4e8012-cd69-424a-b479-8f668c4dc227" />

모든 미션의 개념들을 학습하였고 레벨도 많이 올랐다!

### 느낀점

chatGPT를 이렇게 활용할 수도 있구나 생각하였다. 프롬프트를 어떻게 넣냐에 따라 많은 기능을 활용할 수 있겠구나 생각이 들었다. 이번 퀘스트를 진행하면서 정말 게임을 한 것 같아 신기하다.. 또한, 단순히 개념을 복습하는 것을 넘어 능동적으로 학습 내용을 소화하고 설명하는 연습이 자연스럽게 이루어졌다. 게임의 각 단계를 통과하려면 학습했던 내용을 설명을 해야하는데 설명하면서 확실히 개념을 이해하게 되었던 것 같다.

---

# J228\_임성균

### 선정한 퀘스트 : 3: 무한 퀘스트 생성기 - 다음 챌린지를 부탁해!

### 선정 이유 :

퀘스트를 고민하는 일이 생각보다 쉽지 않아 AI의 도움이 필요했습니다. 주제와 자연스럽게 연결되는 퀘스트 방향을 제안 받고 이를 커뮤니티에 공유하면 좋을 것 같아서 선택하게 되었습니다.

### 진행 방식
chatGPT에게 프롬프트를 전달하여 아래 4가지 퀘스트를 추천받았습니다.

`퀘스트1: 에러 로그 탐정단 - 버그의 흔적을 추적하라`
- 한 줄 요약: 학습 중 마주친 에러 메시지를 AI 탐정과 함께 분석하며, 문제 해결 능력을 레벨업하는 디버깅 퀘스트
- 타입: 디버깅 훈련소 | 문제 해결 | AI 협업
- 목표:
  - 오늘 만난 에러/버그 중 하나를 캡처하거나 설명으로 정리
  - AI에게 ‘탐정’ 역할을 부여하고, 에러의 원인을 함께 추리
  - 해결 과정을 로그 형식으로 정리해 Slack에 공유
- 맥락 연결:
  - 던전앤드래곤에서의 전투 → 디버깅 전투로 확장
  - 학습 내용을 에러와 연결하며 실전 대응력 향상

`퀘스트2: 개념 이주민 센터 - 낯선 개념의 정착을 돕자`
- 한 줄 요약: 아직 익숙하지 않은 개념 하나를 AI와 함께 스터디 자료로 ‘이주’시키는 퀘스트

퀘스트3: 스킬트리 매니저 - 나만의 기술 성장 지도 그리기
- 한 줄 요약: 지금까지 배운 내용을 기반으로 개인 스킬트리 맵을 그리고, 다음 단계 퀘스트를 직접 설계하는 메타 퀘스트
- 타입: 학습 설계 | 지식 정착 | 자료 제작
- 목표:
  - 헷갈리거나 막연한 개념 1개를 고름 (예: Context Switching, DNS, Heap vs Stack)
  - AI와 함께 요약 → 비유 → 그림 → 퀴즈 형태로 자료화
  - 슬랙에 정착지원 카드(슬라이드 or 이미지)로 공유
- 맥락 연결:
  - 알고리즘 미식가에서의 비유적 소화 → 정착 카드로 시각적 학습 보강
  - 다른 사람에게도 전달할 수 있을 만큼 내재화된 개념 만들기

 `퀘스트 3: 스킬트리 매니저 - 나만의 기술 성장 지도 그리기`
- 한 줄 요약: 지금까지 배운 내용을 기반으로 개인 스킬트리 맵을 그리고, 다음 단계 퀘스트를 직접 설계하는 메타 퀘스트
- 타입: 메타 학습 | 로드맵 기획 | 자기 주도
- 목표:
  - 지금까지 학습한 주제 키워드를 정리
  - 그 키워드를 노드로 하여 나만의 스킬트리 or 레벨트리를 만든다
  - 트리 기반으로 다음 목표 2가지를 정하고, 미니퀘스트로 정의
- 맥락 연결:
  - 무한 퀘스트 생성기에서 한 단계 더 확장 → 자기주도형 설계 능력까지 획득
  - CS 개념들을 시각적으로 정리하며 큰 흐름을 파악

`퀘스트 4: AI NPC 시뮬레이터 - 개념 인터뷰를 준비하라`
- 한 줄 요약: 어려운 기술 개념을 AI에게 ‘면접관’ 역할을 맡기고, 질문에 대답하며 지식의 깊이를 점검하는 퀘스트
- 타입: 인터뷰 시뮬레이션 | 학습 점검 | 긴장감 테스트
- 목표:
  - 내가 자신 없는 개념 1~2개를 고름
  - AI에게 “기술 면접관” 역할을 맡기고 질문을 요청
  - 내가 답변 → AI의 피드백 받기 → 수정
- 맥락 연결:
  - 던전 전투 → 면접 전투로 진화
  - 학습의 깊이를 점검하고 실전 준비로 전환
---

# J283\_한지은

### 선정한 퀘스트 : 2: 알고리즘 미식가 - AI의 창의적 비유

### 선정 이유

미션을 수행하다보면 어느새 식사 시간이 훌쩍 다가오곤 합니다. 그때마다 ‘뭘 먹지?’ 고민하고, 검색하고, 고르는 과정이 은근히 번거롭고 학습 흐름이 끊기는 것 같기도 하였습니다. 이 퀘스트는 이런 번거로움을 기분 좋게 해결해줄 수 있을 것 같아 재미있게 느껴졌습니다. 또 이전에 학습한 내용을 기반으로 매뉴를 추천해주니 가볍게 복습도 가능하고, 만약 좋은 비유라면 더 기억에 남을 수 있을 것 같아 선택하였습니다.

### 진행 방식

다음 프롬프트를 ChatGPT에 입력해 메뉴를 추천 받았습니다. 제가 작성한 학습 정리 문서와 키워드를 전달해 이를 잘 반영하는 메뉴 1가지를 추천하도록 하였습니다.

> 너는 '알고리즘 미식가'야. 셰프가 주방에서 최고의 요리를 만들듯, 너는 코드의 세계에서 가장 맛있는 비유를 찾아내지. 내가 딱딱한 기술 문서를 주면, 점심 메뉴로 맛있게 소화시켜 줘. 나의 뇌와 배를 동시에 만족시킬 완벽한 메뉴를 추천해 줘. 학습 키워드의 특징을 잘 반영하는 메뉴 1가지를 추천해줘.
> 
> 
> 학습 정리 문서 링크: https://gist.github.com/hanpengbutt/ad24c66015ec5fc6e8326abb82d1880e
> 학습 키워드: 함수형 프로그래밍, 불변성, 순수 함수
> 

### 결과

> 🍽️ 오늘의 알고리즘 미식가 메뉴 추천
> 
> 
> 메뉴 이름: "비빔밥 정식 – 순수의 조화, 불변의 미학"
> 
> 👨‍🍳 셰프의 설명:
> 
> 비빔밥은 각각의 재료(함수형 프로그래밍의 작은 순수 함수들)가 고유의 맛과 특성을 변형 없이 유지한 채, 한 그릇에서 조화롭게 어우러지는 요리입니다.
> 
> 🥕 불변성 (Immutability):
> 
> 각 나물(재료)은 조리된 이후 그대로 유지되며, 다른 나물의 상태에 영향을 주지 않습니다. 한 번 데쳐진 시금치는 다시 볶아지지 않아요! 이처럼 값은 한 번 결정되면 바뀌지 않죠.
> 
> 🍳 순수 함수 (Pure Function):
> 
> 고추장을 넣고 비비는 행위는 외부 상태에 의존하지 않고 항상 같은 비빔밥 맛을 보장합니다. "밥 + 나물 + 고추장"은 언제나 "그 맛"이 나요. side effect? 없습니다. 그저 맛있는 결과만 있을 뿐.
> 
> 🥗 함수형 프로그래밍 (Functional Programming):
> 
> 각 재료(함수)는 독립적으로 준비되고, 조합(composition)을 통해 최종 결과(비빔밥)를 만듭니다. 모든 것이 선언적이고 예측 가능하며, 복잡함은 깔끔하게 추상화됩니다.
> 
> 📌 결론:
> 
> 함수형 프로그래밍은 마치 비빔밥처럼, 불변의 재료들을 순수한 방식으로 섞어낸 결과입니다. 거기에 한 숟갈의 고추장(고차 함수)이 맛을 완성시켜 주죠.
> 

<img width="750" height="628" alt="image" src="https://github.com/user-attachments/assets/2f648497-9042-47ff-93f9-25767c870162" />

### 느낀점

처음에 제공해주신 예시 프롬프트를 그대로 사용하였는데, 메뉴를 코스식으로 추천해주고 실제 학습 내용과 크게 관련이 없는 메뉴를 추천해주곤 하였습니다.  “메뉴 1가지를 추천하라”고 작성하며 제가 학습한 링크 및 주요 키워드를 프롬프트에 추가하였더니 조금 더 잘 추천해주는 것을 볼 수 있었습니다. 이에 따라 프롬프트의 중요성을 느꼈습니다. 학습 키워드를 비유로 한 번 더 확인할 수 있어 좋았고, 실제 추천 받은 메뉴를 먹고 슬랙에 공유하였는데 비빔밥 재료로 이모티콘을 남겨주시는 동료 분들이 귀엽고 재미있었습니다 ㅎㅎ
